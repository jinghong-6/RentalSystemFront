    ...
    <div class="Orders">
        <div style="height: 100%;">
            <div class="Orders-title">
                <RouterLink to="/admin/Role" class="Orders-title-item">用户</RouterLink>
                <RouterLink to="/admin/Role/Landlord" class="Orders-title-item aa">房东</RouterLink>
            </div>
            ...
            <div class="Role-detail-border">
                <div class="Role-detail">
                    ...
                    <div class="Orders-detail-item" v-for="landlord in Landlord" v-if="showFlag">
                        <div class="Orders-detail-item-img">
                            <img :src="landlord.img_url" alt="">
                        </div>
                        <div class="Orders-detail-item-account">{{ landlord.tele }}</div>
                        <div class="Orders-detail-item-name">{{ landlord.landlord_name }}</div>
                        <div class="Orders-detail-item-money">{{ landlord.money }}</div>
                        <div class="Orders-detail-item-addr">{{ landlord.province }}-{{ landlord.county }}</div>
                        <div class="Orders-detail-item-qq">{{ landlord.qq }}</div>
                        <div class="Orders-detail-item-wechat">{{ landlord.wechat }}</div>
                        <div class="Orders-detail-item-status">{{ landlord.landlord_status }}</div>
                        <div class="Orders-detail-item-register">{{ landlord.register_time }}</div>
                        <div class="Orders-detail-item-take">
                            <div class="Orders-detail-item-take-border">
                                <div class="Orders-detail-item-take-take-down" v-if="landlord.landlord_status == '正常'"
                                    @click="banned(landlord.id)">封禁</div>
                                <div class="Orders-detail-item-take-take-up" v-if="landlord.landlord_status != '正常'"
                                    @click="unbanned(landlord.id)">解封</div>
                            </div>
                        </div>
                    </div>
                </div>
...
import { getAllLandlord, changeLandlordStatus } from '@/api/Admin';
...
function getLandlord() {
    ...
    getAllLandlord(token).then(
        res => {
            if (res.status == 200) {
                console.log(res.data)
                let data = res.data
                if (data.code == "902") {
                    Landlord.value = res.data.data
                    for (let i = 0; i < Landlord.value.length; i++) {
                        if (Landlord.value[i].landlord_status == 0) {
                            Landlord.value[i].landlord_status = "正常"
                        }
                        if (Landlord.value[i].landlord_status == 1) {
                            Landlord.value[i].landlord_status = "封禁"
                        }
                    }
                    showFlag.value = true
                }
...
function banned(id) {
    ...
    changeLandlordStatus(data, token).then(
        res => {
            if (res.status == 200) {
                console.log(res.data)
                let data = res.data
