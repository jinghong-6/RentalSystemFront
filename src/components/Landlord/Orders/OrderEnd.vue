<template>
    <div class="Orders">
        <div class="Orders-title">
            <RouterLink to="/landlord/Orders" class="Orders-title-item ">待付款</RouterLink>
            <RouterLink to="/landlord/Orders/OrderCompete" class="Orders-title-item">待确认</RouterLink>
            <RouterLink to="/landlord/Orders/OrderCompeted" class="Orders-title-item">未开始</RouterLink>
            <RouterLink to="/landlord/Orders/OrderBegin" class="Orders-title-item">进行中</RouterLink>
            <RouterLink to="/landlord/Orders/OrderEnd" class="Orders-title-item aa">已结束</RouterLink>
        </div>
            ...
            <div class="Orders-detail-item" v-for="(order,index) in Orders" v-if="showFlag">
                ...
                <div class="Orders-detail-item-deadtime-border">
                    <div class="Orders-detail-item-deadtime">
                        订单开始时间：<span>{{ order.order_begin_time }}</span>
                    </div>
                    <div class="Orders-detail-item-deadtime">
                        订单支付时间：<span>{{ order.order_pay_time }}</span>
                    </div>
                    <div class="Orders-detail-item-deadtime">
                        订单确认时间：<span>{{ order.order_confirm_time }}</span>
                    </div>
                    <div class="Orders-detail-item-deadtime">
                        订单关闭时间：<span>{{ order.order_close_time }}</span>
                    </div>
                </div>
                <div class="Orders-detail-item-buttom">
                    <div>{{OrderStatus[index]}}</div>
                </div>
            </div>
        </div>
    </div>
...
import { getEndOrderL } from '@/api/Order'
...
function getOrders() {
    ...
    getEndOrderL(data,token).then(
        res => {
            if (res.status == 200) {
                console.log(res.data)
                Orders.value = res.data.data
                if (Orders.value.length > 0 && res.data.code == "902") {
                    for (let i = 0; i < Orders.value.length; i++) {
                        if (Orders.value[i].order_status == "0") {
                            OrderStatus.value.push("订单已结束")
                        }
                        if (Orders.value[i].order_status == "4") {
                            OrderStatus.value.push("商家未确认，订单结束")
                        }
                        if (Orders.value[i].order_status == "5") {
                            OrderStatus.value.push("未支付，订单结束")
                        }
