<template>
    <div class="Orders">
        <div class="Orders-title">
            <RouterLink to="/landlord/Comment" class="Orders-title-item">待评价</RouterLink>
            <RouterLink to="/landlord/Comment/Rated" class="Orders-title-item aa">已评价</RouterLink>
        </div>
...
import { getCommentByUUID, getRatedCommentOrderByLandlordId, setOrderComment } from '@/api/Order'
...
function getOrders() {
    ...
    getRatedCommentOrderByLandlordId(data, token).then(
        res => {
            if (res.status == 200) {
                ...
                if (res.status == 200 && res.data.code == "902") {
                    ...
                    for (let i = 0; i < res.data.data[0].children.length; i++) {
                        if (res.data.data[0].children[i].user_type == "0") {
                            res.data.data[0].children[i].user_type = "用户"
                        }
