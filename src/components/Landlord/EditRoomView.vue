<template>
  <div class="PublishRoom">
    <div class="PublishRoom-border">
      <div class="PublishRoom-border-title">编辑民宿</div>
      ...
      <div class="PublishRoom-border-facility">
        ...
        <div class="PublishRoom-border-facility-items-items">
          <div v-for="(facility, index) in facilityArr" :key="index" class="PublishRoom-border-facility-items-item"
            @click="selectedFacility(facility)" ref="facilityRef">
            <svg :key="index" class="room-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
              :p-id="index" width="200" height="200">
              <path :d="svgpath" v-for="svgpath in svgPaths[index]" fill="#8a8a8a"></path>
            </svg>
            <div class="PublishRoom-border-facility-items-item-text">{{ facility }}</div>
          </div>
        </div>
      ...
      <div class="check-buttom-border">
        <div class="check-buttom" @click="upLoad">保存</div>
      </div>
...
import { getAllProvince, getAllLeaderCityByProvince, getAllCityByLeaderCity, PostBannerPhotos, getCityIdByCity } from '@/api/PublishRoom'
import { getHouseById,UpdateHouse } from '@/api/Room'
    ...
    res => {
      if (res.status == 200) {
        if (res.data.code == '902') {
          ...
          HouseName.value = data.house_name
          detailAddr.value = data.full_address
          HouseSynopsis.value = data.introduce
          getRoomType(RoomType.value.indexOf(data.type))
          priceValue.value = data.price
          maxNum.value = data.max_num
          selectedProvince.value = data.provinceZh
          selectedLeaderCity.value = data.leaderZh
          selectedCity.value = data.cityZh
          ...
          let serviceArr = data.service.split(",")
          for (let index = 0; index < serviceArr.length; index++) {
            selectedFacility(serviceArr[index])
          }
          previewUrls.value = data.img.split(",")

          for (let index = 0; index < previewUrls.value.length; index++) {
            oldImages.value.push(previewUrls.value[index])
          }
        }
      }
    }
...
// 选择基础设施
function selectedFacility(text) {
  const index = selectedfacilityArr.value.indexOf(text);
  let facilityArrIndex = facilityArr.value.indexOf(text)
  if (index === -1) {
    selectedfacilityArr.value.push(text);
    facilityRef.value[facilityArrIndex].style.borderColor = 'rgb(243, 39, 90)'; // Change border color
    // console.log(e.target.style.borderColor)
  } else {
    selectedfacilityArr.value.splice(index, 1);
    facilityRef.value[facilityArrIndex].style.borderColor = 'rgb(172, 172, 172)'; // Restore original color
    // console.log(e.target.style.borderColor)
  }
  ...
  function uploadImage(index) {
    if (index >= selectedImages.value.length) {
      ...
      if (ReturnImgs.value.length != 0 || selectedImages.value.length == 0) {
        let img = oldImages.value.concat(ReturnImgs.value)
        FirstImg.value = img[0]
